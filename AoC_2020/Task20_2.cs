...
namespace AoC_2020
{
    ...
    public class Task20_2
    {
        ...
        public long Task(string input)
        {
            ...
            for (var y = 0; y < width; ++y)
            {
                field[y][x] = field[y][x].RemoveBorders();
            }

            var tileWidth = field[0][0].Image.Length;
            var fullWidth = tileWidth * width;

            var fullImage = new char[fullWidth][];
            for (var i = 0; i < fullWidth; i++)
            {
                fullImage[i] = new char[fullWidth];
            }

            for (var y = 0; y < fullWidth; ++y)
            for (var x = 0; x < fullWidth; ++x)
            {
                var tileXNumber = x / tileWidth;
                var tileX = x % tileWidth;

                var tileYNumber = y / tileWidth;
                var tileY = y % tileWidth;

                fullImage[y][x] = field[tileYNumber][tileXNumber].Image[tileY][tileX];
            }

            var fullTile = new Tile
            {
                Id = 0,
                Image = fullImage
            };

            var monstered = fullTile.Produce().First(x => x.ReplaceMonster());

            return monstered.Image.SelectMany(x => x).Count(c => c == '#');
        }
        ...
        private class Tile
        {
            ...
            public Tile RemoveBorders()
            {
                ...
                {
                    Id = Id,
                    Image = Image.Skip(1).Take(size - 2).Select(x => x.Skip(1).Take(size - 2).ToArray()).ToArray()
                };
            }
            ...
            public bool ReplaceMonster()
            {
                ...
                for (var dx = 0; dx <= size - monster[0].Length; ++dx)
                for (var dy = 0; dy <= size - monster.Length; ++dy)
                {
                    var good = true;
