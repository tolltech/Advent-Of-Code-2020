    ...
    public class Task12
    {
        class Direction
        {
            ...
            public Direction TurnRight(int degrees)
            {
                if (degrees % 90 != 0)
                {
                    throw new Exception("degrees");
                }

                var turnCount = degrees / 90;
                i = (i + turnCount) % directions.Length;
            ...
            public Direction TurnLeft(int degrees)
            {
                ...
                var turnCount = (degrees / 90) % directions.Length;

                return TurnRight((directions.Length - turnCount) * 90);
            }
        }
        ...
        public long Task(string input)
        {
            ...
            foreach (var line in lines)
            {
                ...
                switch (command)
                {
                    ...
                    case "W": point.Y += arg;
                        break;
                    case "L": dir = dir.TurnLeft(arg);
                        break;
                    case "R": dir = dir.TurnRight(arg);
                        break;
                    case "F": var delta = dir.GetDirection();
                        point.X += delta.X * arg;
                        point.Y += delta.Y * arg;
                        break;
