...
namespace AoC_2020
{
    ...
    public class Task21_2
    {
        ...
        public string Task(string input)
        {
            ...
            var food = new List<(HashSet<string> Foreign, HashSet<string> Allergen)>();

            foreach (var line in lines)
            {
                var splits = line.Split("(contains", StringSplitOptions.RemoveEmptyEntries)
                    .Select(x => x.Trim().Trim(')'))
                    .ToArray();

                food.Add(
                    (
                        new HashSet<string>(splits[0].Split(" ", StringSplitOptions.RemoveEmptyEntries).ToArray()),
                        new HashSet<string>(splits[1].Split(", ", StringSplitOptions.RemoveEmptyEntries).ToArray())
                    )
                );
            }

            var allergens = food.SelectMany(x => x.Allergen).Distinct().ToArray();
            ...
            while (allergens.Length > 0)
            {
                ...
                {
                    var foodWithAllergen = food.Where(x => x.Allergen.Contains(allergen)).ToArray();
                    if (foodWithAllergen.Length == 0) continue;

                    var currentForeign = new HashSet<string>(foodWithAllergen[0].Foreign);

                    foreach (var currentFood in foodWithAllergen.Skip(1))
                    {
                        currentForeign.IntersectWith(currentFood.Foreign);
                    }

                    if (currentForeign.Count == 1)
                    {
                        foreach (var item in food)
                        {
                            item.Allergen.Remove(allergen);
                            item.Foreign.Remove(currentForeign.Single());
                        }

                        result.Add((allergen, currentForeign.Single()));
                    }
                }

                allergens = food.SelectMany(x => x.Allergen).Distinct().ToArray();
            }

            return string.Join(",", result.OrderBy(x => x.Item1).Select(x => x.Item2));
        }


        private static IEnumerable<TestCaseData> TestCases()
        {
            yield return new TestCaseData(@"mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
trh fvjkl sbzzf mxmxvkd (contains dairy)
sqjhc fvjkl (contains soy)
sqjhc mxmxvkd sbzzf (contains fish)").Returns("mxmxvkd,sqjhc,fvjkl");
...
sgb mgvfmvp bvvp nxhrrv qbmcl cpgxmh kzz mpqvcfq zvrqj bsmmc kxzdg ncqls fcdgnm vzg cgbj gz hcdchl gnvrcr lfkp mfdnj mbcl lknqg fkczp spzzmx dcvrf jgqvpj txztr bcjz brvbdk gcssxq sznfj pbfjh qds fklh mfp msvjddz tmkfc mcg pldq njpg mhnrqp ctqch dvkbjh nnkrx dpzv hvcrh tlppq snxph zzlvpcq ndzjn (contains dairy, nuts)")
                .Returns("mfp,mgvfmvp,nhdjth,hcdchl,dvkbjh,dcvrf,bcjz,mhnrqp");
        }
    }
}