    ...
    public class Task04_2
    {
        ...
        public int Task(string input)
        {
            ...
            for (var i = 0; i < lines.Length; ++i)
            {
                ...
                {
                    if (required.All(x => fields.ContainsKey(x.Key) && x.Value(fields[x.Key])))
                    {
                        ++acc;
                    }
                ...
                {
                    var pair = split.Split(new[] {":"}, StringSplitOptions.RemoveEmptyEntries);
        ...
        private bool Pid(string x)
        {
            return x.Length == 9 && x.All(char.IsNumber);
        }

        private bool Hgt(string x)
        {
            if (x.Length <= 2)
            {
                return false;
            ...
            var number = x.Substring(0, x.Length - 2);
        ...
        private bool Hcl(string x)
        {
            return x.Length == 7 && x[0] == '#' && x.Skip(1).All(c => (c >= 'a' && c <= 'f') || (c >= '0' && c <= '9'));
        }

        private bool Eyr(string x)
        {
            return int.TryParse(x, out var r) && r >= 2020 && r <= 2030;
        }

        private bool Iyr(string x)
        {
            return int.TryParse(x, out var r) && r >= 2010 && r <= 2020;
        }

        private bool Byr(string x)
        {
            return int.TryParse(x, out var r) && r >= 1920 && r <= 2002;
        }

        private static IEnumerable<TestCaseData> TestCases()
        {
            yield return new TestCaseData(@"pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
").Returns(4);
            yield return new TestCaseData(@"eyr:1972 cid:100
...
").Returns(0);
            yield return new TestCaseData(@"eyr:2027
hcl:#602927
