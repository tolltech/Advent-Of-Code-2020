...
namespace AoC_2020
{
    ...
    public class Task14_2
    {
        ...
        public long Task(string input)
        {
            ...
            var mem = new List<(string Address, long Value)>();
            var mask = new int[36];
            foreach (var line in lines)
            {
                if (line.Contains("mask"))
                {
                    var maskChars = line.Replace("mask = ", string.Empty).Trim().ToCharArray().Reverse().ToArray();
                    mask = new int[36];
                    for (var i = 0; i < maskChars.Length; ++i)
                    {
                        if (int.TryParse(maskChars[i].ToString(), out var parsed))
                        {
                            mask[i] = parsed;
                        }
                        else
                ...
                var mems = line.Replace("mem[", string.Empty).Replace("]", string.Empty)
                    .Split(" = ", StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();

                var address = ToLong(mask, mems[0]);
                var value = (long) mems[1];
        ...
        {
            var acc = 0L;
            foreach (var lChar in longs)
            {
                var permutationsCount = (long) Math.Pow(2, lChar.Count(c => c == 'X'));
                for (var i = 0; i < lChar.Length; ++i)
                {
                    var c = lChar[i];

                    if (c == 'X')
                        acc += (long) Math.Pow(2, i) * permutationsCount / 2;
                    else if (c == '1')
                        acc += (long) Math.Pow(2, i) * permutationsCount;
                }
            }
        ...
        private string ToLong(int[] mask, int address)
        {
            var strMem = Convert.ToString(address, 2).PadLeft(36, '0').ToCharArray().Reverse().ToArray();
            ...
            for (var i = 0; i < 36; ++i)
            {
                ...
                if (bit == 1)
                {
                    strMem[i] = bit.ToString().Single();
                }
            }

            return new string(strMem.Reverse().ToArray());
        }
        ...
        {
            yield return new TestCaseData(@"mask = 000000000000000000000000000000X1001X
mem[42] = 100
mask = 00000000000000000000000000000000X0XX
mem[26] = 1").Returns(208);
            yield return new TestCaseData(@"mask = X011X00011011110001X01010011X0X0X010
mem[35176] = 9976167
mem[39031] = 108553
mem[59131] = 6439623
mem[63499] = 293387351
