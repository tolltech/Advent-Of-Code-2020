...
namespace AoC_2020
{
    ...
    public class Task07_2
    {
        ...
        public int Task(string input, string aim)
        {
            var lines = input.Split(new[] {"\r\n"}, StringSplitOptions.RemoveEmptyEntries);

            var hash = new Dictionary<string, Dictionary<string, int>>();

            foreach (var line in lines)
            {
                var splits = line.Split(new[] {" contain "}, StringSplitOptions.RemoveEmptyEntries);
                var left = new string(splits[0].Replace(" bags", "").Where(char.IsLetter).ToArray());
                var right = splits[1].Split(new[] {","}, StringSplitOptions.RemoveEmptyEntries)
                    .Select(x =>
                    {
                        var witrhoutBag = x
                            .Replace(" bags", "")
                        ...
                        var number = !witrhoutBag.Contains("no other")
                            ? int.Parse(new string(witrhoutBag.TakeWhile(char.IsDigit).ToArray()))
                            : 0;

                        return (witrhoutBag.Replace(number.ToString(), "").Replace(" ", "").Replace(".", "").Trim(), number);
                    })
                    .ToDictionary(x => x.Item1, x => x.number);

                hash.Add(left, right);
            }

            var bags = new[] {(Str: aim.Replace(" ", ""), K: 1)};
            var acc = 0;
            while (true)
            {
                if (bags.Length == 0)
                ...
                var nextLevel = new List<(string, int)>();
                foreach (var bag in bags)
                {
                    if (!hash.TryGetValue(bag.Str, out var children))
                    {
                        continue;
                    }

                    acc += bag.K * children.Sum(x => x.Value);

                    nextLevel.AddRange(children.Select(x => (x.Key, bag.K * x.Value)));
                }

                bags = nextLevel.ToArray();
            }
        ...
        {
            yield return new TestCaseData(@"light red bags contain 1 bright white bag, 2 muted yellow bags.
...
dotted black bags contain no other bags.", "shiny gold").Returns(32);
            yield return new TestCaseData(@"shiny gold bags contain 2 dark red bags.
dark red bags contain 2 dark orange bags.
dark orange bags contain 2 dark yellow bags.
dark yellow bags contain 2 dark green bags.
dark green bags contain 2 dark blue bags.
dark blue bags contain 2 dark violet bags.
dark violet bags contain no other bags.", "shiny gold").Returns(126);
            yield return new TestCaseData(@"posh blue bags contain 5 plaid chartreuse bags, 3 plaid lime bags.
...
plaid green bags contain 4 dark brown bags, 1 shiny orange bag.", "shiny gold").Returns(10219);
        }
    }
}