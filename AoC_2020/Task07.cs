...
namespace AoC_2020
{
    ...
    public class Task07
    {
        ...
        public int Task(string input, string aim)
        {
            ...
            var hash = new ConcurrentDictionary<string, HashSet<string>>();
            ...
            foreach (var line in lines)
            {
                ...
                hash.AddOrUpdate(left, x => new HashSet<string>(right), (s, set) => new HashSet<string>(set.Concat(right)));
            }

            var result = new List<string>();
            var bags = new[] {aim.Replace(" ", "")};
            while (true)
            {
                var thisLevel = new List<string>();
                foreach (var bag in bags)
                {
                    var parents = hash.Keys.Where(x => hash[x].Contains(bag));
                    thisLevel.AddRange(parents);
                }
...
dotted black bags contain no other bags.", "shiny gold").Returns(4);
...
plaid green bags contain 4 dark brown bags, 1 shiny orange bag.", "shiny gold").Returns(142);
        }
    }
}