...
namespace AoC_2020
{
    ...
    public class Task19
    {
        ...
        public long Task(string input)
        {
            ...
            var regExStr = GetRegEx(regexp[0], regexp).Replace(" ", string.Empty);

            var regex = new Regex(regExStr);

            return msgs.Count(x => regex.Matches(x).Any(r => r.Length == x.Length));
        }

        private string GetRegEx((string RegEx, int[] OtherRules) rule, Dictionary<int, (string RegEx, int[] OtherRules)> rules)
        {
            ...
            {
                result = result.Replace(otherRule.ToString(), $"{GetRegEx(rules[otherRule], rules)}");
            }
        ...
        {
            yield return new TestCaseData(@"0: 4 1 5
1: 2 3 | 3 2
2: 4 4 | 5 5
3: 4 5 | 5 4
4: ""a""
...
aabbbbbaabbbaaaaaabbbbbababaaaaabbaaabba").Returns(3);
...
abbaaaabaabbbaaabbbbabaaaaaabaabbabbaabb").Returns(156);
        }
    }
}